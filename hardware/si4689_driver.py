...
class Si4689:
    ...
    def get_dab_signal_strength(self) -> Dict[str, object]:
        ...
        -------------------
        ``rssi``        : int
            Empfangspegel in dBuV (NICHT dBm).  Bereich: −128 … +63.
            Praxis: brauchbares Audio meist erst ab ca. 38 dBuV (standortabhaengig).
        ``snr``         : int
        ...
        return {
            "rssi":        status["rssi"],        # int, −128…+63 dBuV
    ...
    def get_acf_status(self) -> Dict[str, object]:
        """
        DAB_ACF_STATUS (0xC2) – Status der Automatically Controlled Features.

        Zeigt, ob der Chip gerade Soft-Mute / Comfort-Noise anwendet. Damit laesst
        sich bei schwachem Empfang eindeutig erkennen, warum kein Ton kommt: bei
        aktivem Soft-Mute faellt audio_level vom Maximum 0x3FFF (=0 dBFS) Richtung 0.

        Antwort (AN649, Cmd 0xC2):
            RESP6-7  AUDIO_LEVEL[15:0]       0 = stumm, 0x3FFF = 0 dBFS
            RESP8-9  CMFT_NOISE_LEVEL[15:0]  0 = aus,   0x3FFF = 0 dBFS

        Rueckgabe
        ---------
        ``audio_level`` : int    Roh 0..16383
        ``audio_dbfs``  : float  20*log10(audio_level/16383) in dBFS (0.0 = voll)
        ``cmft_level``  : int    Roh-Pegel Comfort-Noise
        ``soft_muting`` : bool   True, wenn audio_level deutlich unter Maximum
                                 (Heuristik: < 0x3F00 → Daempfung aktiv)
        """
        import math
        self._write_command([0xC2, 0x00])
        r = self._read_reply(10)              # 4 Status + RESP4..RESP9
        audio_level = r[6] | (r[7] << 8)
        cmft_level  = r[8] | (r[9] << 8)
        audio_dbfs  = (20.0 * math.log10(audio_level / 16383.0)
                       if audio_level > 0 else -99.0)
        return {
            "audio_level": audio_level,
            "audio_dbfs":  round(audio_dbfs, 1),
            "cmft_level":  cmft_level,
            "soft_muting": audio_level < 0x3F00,
        }

    def get_ber_info(self) -> Dict[str, object]:
        """
        DAB_TEST_GET_BER_INFO (0xE8) – Bitfehlerstatistik.

        ACHTUNG: Liefert nur im BER-TEST-SETUP sinnvolle Werte. Erfordert
        DAB_TEST_BER_CONFIG (Property 0xE800) mit Testmuster in einem Subkanal
        UND eine dafuer gebaute Firmware (Paketmodus, TMID=3). Fuer normalen
        Off-Air-Empfang ist der Wert NICHT aussagekraeftig – dort sind cnr
        (aus dab_digrad_status) und audio_level/soft_muting (aus get_acf_status)
        die praktischen Qualitaetsindikatoren.

        Antwort (AN649, Cmd 0xE8):
            RESP4-7   ERR_BITS[31:0]    Anzahl fehlerhaft empfangener Bits
            RESP8-11  TOTAL_BITS[31:0]  Gesamtzahl empfangener Bits

        Rueckgabe
        ---------
        ``err_bits``   : int
        ``total_bits`` : int
        ``ber``        : float   err_bits / total_bits (0.0, wenn total_bits = 0)
        """
        self._write_command([0xE8, 0x00])
        r = self._read_reply(12)              # 4 Status + RESP4..RESP11
        err_bits   = int.from_bytes(bytes(r[4:8]),  "little")
        total_bits = int.from_bytes(bytes(r[8:12]), "little")
        ber = (err_bits / total_bits) if total_bits else 0.0
        return {"err_bits": err_bits, "total_bits": total_bits, "ber": ber}
    ...
    def fm_rsq_status(self, stcack: bool = False) -> Dict[str, object]:
        ...
        ``valid`` : bool
            True = Signal gültig.
        ``rssi`` : int
            Empfangspegel in dBuV (NICHT dBm).
        ``snr`` : int
